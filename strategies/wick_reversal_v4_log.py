"""
Wick Reversal v4 — Log / Percentage-based edition

策略流程與 v4 相同，但將所有依賴「絕對點數」的門檻重構為「相對於當下價格的
百分比」，以跨越數倍的價格尺度仍能維持一致的風險特性（適用 BTC 20k → 100k）。

主要修改點（對照 wick_reversal_v4.py）：
  1. 新增 K0 最小實質波動過濾 min_rng_pct：K 棒 range 必須 > close * min_rng_pct
     ，否則視為低價區雜訊而被濾除，避免 1 分鐘假反轉大量誤觸發。
  2. 移除固定 long_sl_offset / short_sl_offset（10.0 絕對點數）→ 改為 sl_offset_pct
     ，讓停損讓點隨 k0.close 縮放（long: k0.low - k0.close * sl_offset_pct）。
  3. 新增 use_dynamic_vol：啟用時改用最近 k0_vol_sma_period 根 K 棒的 SMA 成交量
     當 k0 偵測動態門檻（k.volume > sma * k0_vol_sma_mult），避開不同年度牛熊市
     的絕對成交量差異造成的策略失效；關閉時退回原本固定門檻。
  4. cost filter（_risk_covers_cost）本身邏輯未動：
        min_risk = round_trip_cost(entry_price) * fee_cover_ratio / rr
        min_risk_pct = min_risk / entry_price = 2*(fee+slip) * ratio / rr（價格無關）
     搭配新的百分比 sl_offset 後，risk / entry_price 也是價格無關比例，
     因此高/低價區間的最小風險門檻與實際風險兩者皆已對齊。

Trailing / TS / TP / TD / cost-filter 架構與 v4 完全一致，未改動。
"""
...
@register
class WickReversalV4LogStrategy(StrategyBase):
    name = "Wick Reversal 1m v4 Log"
    allow_bar_fallback_in_tick_mode: bool = True

    # ── 百分比化共用參數（Log edition 新增）────────────────────────────────
    min_rng_pct: float = 0.0005                     # k0 最小有效 range (占 close 比例；預設 5bps)
    sl_offset_pct: float = 0.0002                   # 動態停損讓點 (占 k0.close 比例；預設 2bps)
    # ── 動態 k0 成交量門檻 ────────────────────────────────────────────────
    use_dynamic_vol: bool = True                    # True=用 SMA 動態門檻；False=退回固定 *_k0_vol_gate
    k0_vol_sma_period: int = 20                     # k0 vol SMA 窗期
    k0_vol_sma_mult: float = 1.0                    # k0 vol SMA 倍率 (k.volume > sma * mult)
    ...
    long_k0_vol_gate: float = 500.0                 # 僅 use_dynamic_vol=False 時生效的固定 k0 成交量門檻
    long_delta_eff_threshold: float = 0.8           # 進場 delta_eff 門檻
    long_vol_sma_period: int = 20                   # 進場成交量 SMA 窗期；0=不過濾
    long_vol_sma_mult: float = 1.0                  # 進場成交量門標倍率
    ...
    short_k0_vol_gate: float = 300.0                # 僅 use_dynamic_vol=False 時生效的固定 k0 成交量門檻
    short_delta_eff_threshold: float = 0.8          # 進場 delta_eff 門檻（負向）
    short_vol_sma_period: int = 20                  # 進場成交量 SMA 窗期；0=不過濾
    short_vol_sma_mult: float = 1.2                 # 進場成交量門標倍率
    ...
    # ══════════════════════════════════════════════════════════════════════════
    # 動態門檻 helper（Log edition 新增）
    # ══════════════════════════════════════════════════════════════════════════
    def _long_sl_offset(self, k0: Kline) -> float:
        """動態停損讓點：以 k0.close 為基準。"""
        return k0.close * self.sl_offset_pct

    def _short_sl_offset(self, k0: Kline) -> float:
        """動態停損讓點：以 k0.close 為基準。"""
        return k0.close * self.sl_offset_pct

    def _k0_min_rng_ok(self, k: Kline) -> bool:
        """K 棒最小實質波動：rng 必須 > close * min_rng_pct。"""
        if self.min_rng_pct <= 0:
            return True
        return (k.high - k.low) > k.close * self.min_rng_pct

    def _k0_vol_gate_ok(self, k: Kline, klines: List[Kline], cur_idx: int,
                        static_gate: float) -> bool:
        """
        k0 成交量門檻（偵測時）：
          use_dynamic_vol=True → 要求 k.volume > SMA(前 N 根) * mult
                                 若樣本不足則先放行（與 _vol_sma_ok 行為一致）。
          use_dynamic_vol=False → 退回固定 static_gate（long/short 各自一個）。
        """
        if not self.use_dynamic_vol:
            return k.volume >= static_gate
        period = max(self.k0_vol_sma_period, 0)
        if period <= 0 or cur_idx < period:
            return True
        s = cur_idx - period
        sma = sum(klines[j].volume for j in range(s, cur_idx)) / period
        return k.volume > sma * self.k0_vol_sma_mult
    ...
    def on_history(
        ...
        for i, k in enumerate(klines):
            ...
            if not in_position:
                ...
                # context（同時供動態 vol gate 與 _short_k0_regime_ok 使用；
                #  移到 long/short 偵測前以便 _is_k0_long 也能使用）
                self._cur_i = i
    ...
    def _bar_entry(
        ...
        停損掛在 k0 K 棒最低點下方（動態讓點 = k0.close * sl_offset_pct）。
        """
        ...
        stop_p = k0.low - self._long_sl_offset(k0)          # Log edition：動態讓點
        rr = self._resolve_long_rr(k0)
        risk = entry_p - stop_p
        if risk <= 0:
            return False, 0.0, 0.0, 0.0
        # cost filter：min_risk 本身就是 entry_price 的固定百分比，
        # 與百分比 sl_offset 搭配後，高/低價區行為一致。
    ...
    def _tick_entry(
        ...
        守護線為 k0 實體低點；停損掛在 k0.low 下方（動態讓點）。
    ...
    def _is_k0_long(
        ...
        if rng <= 0:
            return False
        # Log edition：最小實質波動（百分比）— 濾除低價區雜訊 K 棒
        if not self._k0_min_rng_ok(k):
            return False
        # Log edition：動態 / 固定 成交量門檻（使用 _cur_klines / _cur_i）
        if not self._k0_vol_gate_ok(
            k,
            getattr(self, "_cur_klines", []),
            getattr(self, "_cur_i", 0),
            self.long_k0_vol_gate,
        ):
            return False
    ...
    def _risk_covers_cost(self, entry_price: float, risk: float, rr: float, fee_cover_ratio: float) -> bool:
        """
        min_risk = round_trip_cost(entry_price) * fee_cover_ratio / rr
                 = 2*(fee+slip) * entry_price * fee_cover_ratio / rr
        → min_risk / entry_price 為固定比例（價格無關），因此搭配百分比 sl_offset 後，
          高/低價區的風險門檻與實際風險兩者皆為等比縮放，校準一致。
        """
    ...
    def _is_k0_short(
        ...
        做空 k0（做多的鏡像）：
          ...
          - Log edition：min_rng_pct 百分比過濾 + 動態 vol gate
        """
        rng = k.high - k.low
        if rng <= 0:
            return False
        # Log edition：最小實質波動（百分比）
        if not self._k0_min_rng_ok(k):
            return False
        # Log edition：動態 / 固定 成交量門檻
        if not self._k0_vol_gate_ok(
            k,
            getattr(self, "_cur_klines", []),
            getattr(self, "_cur_i", 0),
            self.short_k0_vol_gate,
        ):
            return False
        mid = (k.high + k.low) / 2.0
    ...
    def _bar_entry_short(
        ...
        停損掛在 k0 K 棒最高點上方（動態讓點 = k0.close * sl_offset_pct）。
        """
        ...
        entry_p = k0_body_low
        stop_p  = k0.high + self._short_sl_offset(k0)       # Log edition：動態讓點
    ...
    def _tick_entry_short(
        ...
        累計 delta_eff < -threshold 時入場。
        守護線為 k0 實體高點；停損掛在 k0.high 上方（動態讓點）。
        """
        k0_body_low  = min(k0.open, k0.close)
