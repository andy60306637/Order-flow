"""
WickReversalV4 SHORT parameter grid search optimizer.
...
GRID = {
    "short_k0_vol_gate":           [200.0, 300.0],
    "short_vol_sma_mult":          [1.2, 1.5],
    "short_rr_wick_a":             [3.5, 4.0, 4.5],
    "short_rr_wick_b":             [2.0, 2.5],
    "short_rr_wick_c":             [1.5, 2.0],
    "short_delta_eff_threshold":   [0.6, 0.8, 1.0], # 這裡代碼內部會取負值，所以填正數
}
...
def run_one(strategy_cls, klines, tick_map, params: dict) -> dict | None:
    strat = strategy_cls()
    strat.enable_long = False
    strat.enable_short = True
    
    # 固定一些從 Long 優化中學到的優質基準
    strat.short_sl_offset = 10.0
    strat.short_min_fee_cover_ratio = 2.0
    strat.short_zoom_bars = 1
    
    for k, v in params.items():
        setattr(strat, k, v)
    
    # Sync internal fee rates
