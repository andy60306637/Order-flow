...
│   └── TradingPipeline                                                │
│       ├── RegimeStage(regime_comp, allowed=["trending_bull"])        │
│       ├── SessionStage(session_comp, allowed=["london","ny"])        │
...
- 讀取 `SharedContext["regime"]`，不符合白名單則阻斷。
- 填入 `ctx.regime`、`ctx.regime_meta`。
...
- 讀取 `SharedContext["session"]`，不符合白名單則阻斷。
- 結果附加至 `ctx.regime_meta["session"]`（供日誌讀取）。
...
pipeline = TradingPipeline([
    RegimeStage(
        component=regime_comp,
        allowed_regimes=["trending_bull", "trending_bear"],
    ),
    SessionStage(
        component=session_comp,
        allowed_sessions=["london", "ny", "overlap"],
    ),
    AlphaStage(
        modules=[wick_signal],
...
```
1. RegimeStage / SessionStage   ← 最輕量的過濾，放最前
